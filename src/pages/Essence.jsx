...
/** Handles / titles / tags (lowercase). Override with VITE_ESSENCE_COLLECTION_HANDLES=e.g. essence,my-collection-handle */
const ESSENCE_KEYS = new Set(
  String(import.meta.env.VITE_ESSENCE_COLLECTION_HANDLES || 'essence,essences')
    .split(',')
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean)
)

const productBelongsOnEssencePage = (p) => {
  const handles = (p.collectionHandles || []).map((h) =>
    String(h).toLowerCase()
  )
  if (handles.some((h) => ESSENCE_KEYS.has(h))) return true
  const titles = (p.collection || []).map((t) => String(t).trim().toLowerCase())
  if (titles.some((t) => ESSENCE_KEYS.has(t))) return true
  const tags = (p.tags || []).map((t) => String(t).trim().toLowerCase())
  if (tags.some((t) => ESSENCE_KEYS.has(t))) return true
  return false
}
...
const Essence = () => {
  ...
  const essenceProducts = useMemo(
    () => products.filter(productBelongsOnEssencePage),
    [products]
  )

  const filterGroups = useMemo(
    () => buildFilterGroups(essenceProducts),
    [essenceProducts]
  )
  ...
  const visibleProducts = useMemo(() => {
    ...
    const filtered = essenceProducts.filter(
      (product) =>
